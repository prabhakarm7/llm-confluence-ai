    ...
    async def get_filter_options(self) -> FilterOptions:
        ...
        with neo4j_conn.get_session() as session:
            ...
            try:
                ...
                # Get distinct PCAs (handle both string and array properties)
                result = session.run("""
                    MATCH (n) 
                ...
                pca_set = set()
                for record in result:
                    pca_value = record["pca"]
                    if pca_value is not None:
                        if isinstance(pca_value, list):
                            # If it's a list, add each non-empty item
                            for item in pca_value:
                                if item and str(item).strip():
                                    pca_set.add(str(item).strip())
                        elif str(pca_value).strip():
                            # If it's a string and not empty
                            pca_set.add(str(pca_value).strip())
                
                filter_options["pca_options"] = sorted(list(pca_set))
                print(f"Found {len(filter_options['pca_options'])} PCAs: {filter_options['pca_options']}")
                
                # Get distinct ACAs (handle both string and array properties)  
                result = session.run("""
                    MATCH (n) 
                ...
                aca_set = set()
                for record in result:
                    aca_value = record["aca"]
                    if aca_value is not None:
                        if isinstance(aca_value, list):
                            # If it's a list, add each non-empty item
                            for item in aca_value:
                                if item and str(item).strip():
                                    aca_set.add(str(item).strip())
                        elif str(aca_value).strip():
                            # If it's a string and not empty
                            aca_set.add(str(aca_value).strip())
                
                filter_options["aca_options"] = sorted(list(aca_set))
                print(f"Found {len(filter_options['aca_options'])} ACAs: {filter_options['aca_options']}")
